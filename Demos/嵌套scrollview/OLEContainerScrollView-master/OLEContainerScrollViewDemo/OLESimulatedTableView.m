...
- (void)layoutSubviews
{
    ...
    [self.cells enumerateObjectsUsingBlock:^(OLESimulatedTableViewCell *cell, NSUInteger idx, BOOL *stop) {
        CGFloat cellYPosition = self->_edgeInsets.top + (idx * (self->_rowHeight + self->_cellSpacing));
        cell.frame = CGRectMake(self->_edgeInsets.left, cellYPosition, cellWidth, self->_rowHeight);
