...
- (void)setText:(NSString *)text{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        if ([temp isEqualToString:text]) {
            ...
            CGSize shotimageSize = screenShotimage.size;
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if (drawFlag==flag) {
                    if (isHighlight) {
                        if (highlighting) {
                            highlightImageView.image = nil;
                            if (highlightImageView.width!=shotimageSize.width) {
                                highlightImageView.width = shotimageSize.width;
                            }
                            if (highlightImageView.height!=shotimageSize.height) {
                                highlightImageView.height = shotimageSize.height;
                            }
                            highlightImageView.image = screenShotimage;
                        }
                    } else {
                        if ([temp isEqualToString:text]) {
                            if (labelImageView.width != shotimageSize.width) {
                                labelImageView.width = shotimageSize.width;
                            }
                            if (labelImageView.height != shotimageSize.height) {
                                labelImageView.height = shotimageSize.height;
                            }
                            highlightImageView.image = nil;
                            labelImageView.image = nil;
