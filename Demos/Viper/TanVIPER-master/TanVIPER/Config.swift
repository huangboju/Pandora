...
class View: ViewController, PresenterToViewPipline {
    ...
    func display(viewModel _: ViewModel) {
        fatalError("display(viewModel:) is an abstract function")
    }

    func show(router _: Router, userInfo _: Any?) {
...
class Interactor: ViewToInteratorPipline {
    ...
    func refresh(request _: Request) {
...
class Presenter: InteratorToPresenterPipline {
    ...
    func present(response _: Response) {
...
class Router {
    ...
    func route(type: RouteType, userInfo: Any?) {
        ...
        case .push:
            if let view = view {
                presenter?.view?.navigationController?.pushViewController(view, animated: true)
            }
        case .modal:
            if let view = view {
                presenter?.view?.present(view, animated: true, completion: nil)
            }
        case .back:
