...
-(void)setupWithIPAddress:(NSString *) IPAdress Block:(StartupCallback)callback {
    self.hostAddressString = IPAdress;

    [self setupWithBlock:callback];
}
...
//        }
        ...
        BOOL _isIPV6 = [self.hostAddressString rangeOfString:@":"].location == NSNotFound ? NO : YES;
        ...
        if (_isIPV6) {
            ...
            nativeAddr6.sin6_port = htons(80);
            inet_pton(AF_INET6, self.hostAddressString.UTF8String, &nativeAddr6.sin6_addr);
            self.hostAddress = [NSData dataWithBytes:&nativeAddr6 length:sizeof(nativeAddr6)];
        } else {
            struct sockaddr_in nativeAddr4;
            memset(&nativeAddr4, 0, sizeof(nativeAddr4));
            nativeAddr4.sin_len = sizeof(nativeAddr4);
            nativeAddr4.sin_family = AF_INET;
            nativeAddr4.sin_port = htons(80);
            inet_pton(AF_INET, self.hostAddressString.UTF8String, &nativeAddr4.sin_addr.s_addr);
            self.hostAddress = [NSData dataWithBytes:&nativeAddr4 length:sizeof(nativeAddr4)];
        }
            ...
            if (pingSummary) {
                if ([self isValidPingResponsePacket:packet]) {
                    ...
                    if (self.delegate && [self.delegate respondsToSelector:@selector(ping:didReceiveReplyWithSummary:)] ) {
                        dispatch_async(dispatch_get_main_queue(), ^{
                            //notify delegate
                            pingSummary.status = GBPingStatusTimeout;
...
-(void)sendPing {
    if (self.isPinging) {
        ...
        if (self.socket == 0) {
            ...
            newPingSummary.host = self.hostAddressString;
            ...
            NSBlockOperation *target = [NSBlockOperation blockOperationWithBlock:^{
