        ...
        MobileNetCode:(NSString *)theMobileNetCode
{
    ...
    if (self) {
        ...
        _fastSummary = [[LDNetSummary alloc] init];
        _fastSummary.averageTime = 10000;
    }
...
- (id)initWithAppCode:(NSString *)theAppCode
               userID:(NSString *)theUID
             ipAddress:(NSArray <NSString *>*)ipAddress
     maximumPingTimes:(int)times {
    NSString *version =[[[NSBundle mainBundle] infoDictionary] objectForKey:@"CFBundleShortVersionString"];
    self = [self initWithAppCode:theAppCode appName:theUID appVersion:version userID:theUID deviceID:nil dormain:nil carrierName:nil ISOCountryCode:nil MobileCountryCode:nil MobileNetCode:nil];
    _hostAddress = ipAddress;
    _maximumPingTimes = times;
    return self;
}
...
- (void)startNetDiagnosis
{
    ...
    if (_curNetType == 0) {
        ...
        [self recordStepInfo:@"\n当前主机未联网，请检查网络！"];
        [self recordStepInfo:@"\n网络诊断结束\n"];
        if (self.delegate && [self.delegate respondsToSelector:@selector(netDiagnosisDidEnd:fastIPSummary:)]) {
            [self.delegate netDiagnosisDidEnd:_logInfo fastIPSummary:nil];
        }
        return;
    }
...
//        [self recordOutIPInfo];
    ...
    if (_isRunning) {
        ...
        if ([_hostAddress count] > 0) {
            _netConnect = [[LDNetConnect alloc] initWithMaximumPingTimes:_maximumPingTimes];
...
- (void)traceRouteDidEnd {
    ...
    [self recordStepInfo:@"\n网络诊断结束\n"];
    if (self.delegate && [self.delegate respondsToSelector:@selector(netDiagnosisDidEnd:fastIPSummary:)]) {
        [self.delegate netDiagnosisDidEnd:_logInfo fastIPSummary: _fastSummary];
    }
}

#pragma mark - connectDelegate

- (void)appendSocketLog:(NSString *)socketLog summary:(LDNetSummary *)summary {
    if (_fastSummary.averageTime > summary.averageTime && summary.ipAddress.length > 0) {
        _fastSummary.averageTime = summary.averageTime;
        _fastSummary.ipAddress = summary.ipAddress;
    }
