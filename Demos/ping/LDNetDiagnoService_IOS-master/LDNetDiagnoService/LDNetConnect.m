...
@interface LDNetConnect () {
    ...
    NSInteger _sumTime;
    CFSocketRef _socket;
    int MAXCOUNT_CONNECT;
    
    LDNetSummary *_summary;
}
...
- (id)initWithMaximumPingTimes:(int)maximumPingTimes {
    if (self = [super init]) {
        if (maximumPingTimes <= 0) {
            MAXCOUNT_CONNECT = 4;
        } else {
            MAXCOUNT_CONNECT = maximumPingTimes;
        }
        _summary = [[LDNetSummary alloc] init];
    }
    return self;
}
...
- (void)runWithHostAddress:(NSString *)hostAddress port:(int)port
{
    ...
    _sumTime = 0;
    _resultLog = @"";
    if (self.delegate && [self.delegate respondsToSelector:@selector(appendSocketLog:summary:)]) {
        [self.delegate appendSocketLog:[NSString stringWithFormat:@"connect to host %@ ...", _hostAddress] summary:_summary];
    }
...
- (void)readStream:(BOOL)success
{
    ...
    if (_connectCount == MAXCOUNT_CONNECT - 1) {
        if (_sumTime >= 99999) {
            ...
            _summary.ipAddress = _hostAddress;
            _summary.averageTime = _sumTime / MAXCOUNT_CONNECT;
            _resultLog = [_resultLog
                stringByAppendingString:[NSString stringWithFormat:@"average=%ldms", (long)(_summary.averageTime)]];
        }
        if (self.delegate && [self.delegate respondsToSelector:@selector(appendSocketLog:summary:)]) {
            [self.delegate appendSocketLog:_resultLog summary:_summary];
        }
    }
