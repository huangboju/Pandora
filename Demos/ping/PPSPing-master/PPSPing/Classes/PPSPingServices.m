...
- (instancetype)initWithAddresses:(NSArray<NSString *> *)addresses maximumPingTimes:(NSInteger)count {
    self = [super init];
    if (self) {
        _maximumPingTimes = count;
        _ipAddress = addresses;
    }
    return self;
}
...
- (void)startPingGroup {
//    for (NSString *ipAddress in _ipAddress) {
//        
//    }
}
...
- (NSString *)statisticsPingInfo {
    ...
    for (PPSPingSummary *item in _pingItems) {
        if (item.rtt > 0) {
            [pings addObject:item];
            countTime += item.rtt;
        }
    }
    NSNumber *averageTime = [pings valueForKeyPath:@"@avg.rtt"];
    NSNumber *maxTime = [pings valueForKeyPath:@"@max.rtt"];
    NSNumber *minTime = [pings valueForKeyPath:@"@min.rtt"];
    NSString *resultString = [NSString stringWithFormat:@"\n最大响应时间：%@\n最小响应时间：%@\n平均响应时间：%@",maxTime,minTime, averageTime];
    return resultString;
}
//
